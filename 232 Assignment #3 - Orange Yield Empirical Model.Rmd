...
prcp_select <- prcp_select[-c(1:3,5)]

prcp_final <- prcp_select %>% 
  select(year, everything()) # final dataframe for precip for month of may for every year

write.csv(x=prcp_final, file ="caladapt_precip_final.csv")
...
write.csv(x=tmin_final, file ="caladapt_tmin_final.csv")
```
