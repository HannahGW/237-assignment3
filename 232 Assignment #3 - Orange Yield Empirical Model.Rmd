...
```{r}
result_future <- orange_anomaly(precip = prcp_final$prcp, min_temp = tmin_final$tmin)

combine <- merge(tmin_final, prcp_final)

end <- data.frame(result, combine)

options(scipen = 999)

ggplot(end, aes(x = tmin, y = result)) +
  geom_point()
```
